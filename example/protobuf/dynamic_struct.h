...
// class BaseSchema;
// typedef babylon::FieldAccessor FieldAccessor;
...
//     template <typename A, typename ::std::enable_if<::std::is_constructible<std::allocator<DynamicStruct, A>, A*>::value, int32_t>::type = 0>
...
//     inline static DynamicStruct* create_with_pool(const Schema&, std::allocator<DynamicStruct>);
// #ifdef GOOGLE_PROTOBUF_HAS_ARENAS
//     inline static DynamicStruct* create_with_pool(const Schema&, std::allocator<DynamicStruct>);
//     inline static DynamicStruct* create_with_metadata(
//             const Schema&,
//             std::allocator<DynamicStruct>,
...
// template <typename A>
// class ReuseDynamicStructPool {
// public:
//     typedef std::vector<DynamicStruct*, std::allocator<DynamicStruct*, A>>
//             ReusableVectorDynamicStruct;
//     inline ReuseDynamicStructPool(const DynamicStruct::Schema* schema, A* pool) :
//             _schema(schema),
//             _pool(pool),
//             _ds_vec(std::allocator_arg, MonotonicAllocator<DynamicStruct*, A>(*pool)) {}
//     ReuseDynamicStructPool() = delete;
//     ReuseDynamicStructPool(ReuseDynamicStructPool&) = delete;
//     ReuseDynamicStructPool(ReuseDynamicStructPool&&) = delete;
//     ReuseDynamicStructPool& operator=(ReuseDynamicStructPool&) = delete;
//     ReuseDynamicStructPool& operator=(ReuseDynamicStructPool&&) = delete;
//     //线程不安全
//     inline DynamicStruct* get();
...
//         return _ds_vec.size();
//     }
//     inline ~ReuseDynamicStructPool() {
//         if (!_schema->is_trivally_destructiable()) {
//             for (auto& ds : _ds_vec) {
//                 ds->~DynamicStruct();
//             }
//         }
//     }
//     struct AllocationMetadata {
//         DynamicStruct::AllocationMetadata ds_metadata;
//         size_t size;
//     };
//     //重建协议
//     typedef MonotonicAllocator<ReuseDynamicStructPool<A>, A> allocator_type;
//     inline ReuseDynamicStructPool(
//             ::std::allocator_arg_t,
//             allocator_type,
//             const AllocationMetadata&);
//     inline size_t update_allocation_metadata(AllocationMetadata&) const;
//     inline void reconstruct();
// private:
//     const DynamicStruct::Schema* _schema;
//     A* _pool;
//     ReusableVectorDynamicStruct _ds_vec;
//     size_t use_num{0};
// };
