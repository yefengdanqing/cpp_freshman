...
template <typename A, typename ::std::enable_if<::std::is_constructible<MonotonicAllocator<DynamicStruct, A>, A*>::value, int32_t>::type>
inline DynamicStruct* DynamicStruct::create_with_pool(
        const Schema& schema,
        A* resource) {
    auto allocator = MonotonicAllocator<DynamicStruct, A>(resource);
...
inline ReuseDynamicStructPool<A>::ReuseDynamicStructPool(
        ...
        const AllocationMetadata& metadata) :
        _ds_vec(std::allocator_arg, MonotonicAllocator<DynamicStruct*, A>(*allocator.underlying)) {
