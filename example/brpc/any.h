...
public:
    // 引用到Any内部，兼容之前的用法
    template <typename T>
    using TypeId = ::baidu::feed::mlarch::babylon::TypeId<T>;
    typedef ::baidu::feed::mlarch::babylon::Id Id;
    ...
    template <typename T, typename ::std::enable_if<sizeof(size_t) < sizeof(T), int32_t>::type = 0>
    inline explicit Any(T&& value);
    // 特化支持8字节以内小对象，内联存储
    template <typename T, typename ::std::enable_if<sizeof(T) <= sizeof(size_t), int32_t>::type = 0>
    inline explicit Any(T&& value);
    ...
    template <typename T>
    int32_t to(T& value) const noexcept;
...
private:
    ...
    struct TypeDescriptor<T, typename ::std::enable_if<::std::is_copy_constructible<T>::value>::type> {
        ...
        static constexpr Descriptor descriptor {
            .type_id = TypeId<T>().ID,
            .destructor = TypeDescriptor<T, int32_t>::destructor,
            .deleter = TypeDescriptor<T, int32_t>::deleter,
